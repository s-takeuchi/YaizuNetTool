...
void ChangeFlag(int CurrentId, int Type, HINSTANCE InstHndl, HWND WndHndl, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if (message == WM_CREATE) {
		...
		SelectedVarId = LowDbAccess::GetInstance()->GetElementInfoParamInt(CurrentId, 1);
		MaxIndex = StkProp_CollectAndSetFlagVariable(ChgVar, VarId, SelectedVarId);

		SelectedValue = LowDbAccess::GetInstance()->GetElementInfoParamInt(CurrentId, 2);
		if (SelectedValue == 1) {
	...
	if (message == WM_COMMAND) {
		...
		if (HIWORD(wParam) == BN_CLICKED) {
			...
			if (LOWORD(wParam) == IDC_BTNOK) {
				LowDbAccess::GetInstance()->SetElementInfoParamInt(CurrentId, SelectedVarId, 1);
				LowDbAccess::GetInstance()->SetElementInfoParamInt(CurrentId, SelectedValue, 2);
			}
		}
	}
}
